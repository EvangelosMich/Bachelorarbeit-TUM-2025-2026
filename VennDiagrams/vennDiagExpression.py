...
df2 = pd.read_csv(os.path.join(output_folder, "ExpressionProcessed.csv"))
...
for col1, col2 in zip(numeric_cols1, numeric_cols2):
    ...
    plt.title(f"Feature Matching: {col2}")
...
dfResult[any_match_mask].to_csv(os.path.join(output_folder, "processedDf2_Final_Aligned.csv"), index=False)
